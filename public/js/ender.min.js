...
/*!
  ...
  */
;!function(a){a.ender({poke:function(b){return this.forEach(function(c){function i(a,b,c,d){var e=Math.abs(a-b),f=Math.abs(c-d);return e>=f?a-b>0?"W":"E":c-d>0?"N":"S"}var d=a(c),e,f={},g,h;d.bind(b).bind({touchstart:function(a){e=[];g="";h=a.touches.length-1;f.x1=a.touches[h].pageX;f.y1=a.touches[h].pageY},touchmove:function(a){f.x2=a.touches[h].pageX;f.y2=a.touches[h].pageY;if(Math.abs(f.x1-f.x2)>60||Math.abs(f.y1-f.y2)>60){var b=i(f.x1,f.x2,f.y1,f.y2);f.x1=f.x2;f.y1=f.y2;if(b==g)return;g=b;e.push(b)}},touchend:function(){if(!e.length)return;d.trigger(e.join("-"))}})})}},!0)}(ender);provide("ender-poke",a.exports)})()